...
top10city <- expenditure %>% 
  ...
  filter(year == 2023, area_code != "00000") %>% 
  select(city, value) %>% 
  arrange(desc(value)) %>% 
  slice_head(n = 10)
...
Miyazaki-city won in 2021 and 2022. Hamamatsu-city won in 2023. The battle between Utsunomiya-city and Hamamatsu-city has a long history. Let us draw all available years of top 10 cities in 2023.
...
Hamamatsu-city had been the top of eating the grilled eel in 11 years in a row up to 2018. It lost the top position in 2019, regained it in 2020, lost again in 2021, remained second in 2022, and regained it in 2023.
...
I count the number of cities which get the top rank in 17 years from 2007 to 2023 for each item.
...
In how many items among `r lookup_cat01 %>% filter(level == 5) %>% nrow()` did each city get the top in 2023?
...
I draw the histogram that shows each city's ranks of `r lookup_cat01 %>% filter(level == 5) %>% nrow()` items in 2023. Naha-city and other Kyushu cities and Wakayama-city tend to rank low, while Tokyo and other Kanto cities tend to rank high.
...
area_ranks %>% 
  ...
  labs(title = "Ranks distribution in 52 cities in 2023")
...
national <- expenditure %>% 
  ...
  filter(year == 2023, area_code == "00000", cat02_code == "03") %>% 
  select(value) %>%
  as.numeric()
