...
I hear which city in Japan consumed most dumplings (gyouza in Japanese, and jiaozi in Chinese) per household last year. That is an annual routine news report. Hamamatsu-city declared victory in February 2019 [(Japanese)](https://prtimes.jp/main/html/rd/p/000000009.000038874.html).
...
I find that persistent winning items are speciality goods of each city. I list the top earners in 2018. I draw ranks distribution in each city in 2018. And I argue ranks distribution is partly due to total expenditure differences.
...
```
I search around in Family Income and Expenditure Survey, and manage to know the statsDataId of the appropriate table is "0003125169" from [this page (Japanese)](https://www.e-stat.go.jp/stat-search/database?page=1&layout=datalist&toukei=00200561&tstat=000000330001&cycle=7&tclass1=000000330001&tclass2=000000330004&statdisp_id=0003125169&result_page=1&second2=1). As I mentioned in the warning in the top, I failed to set lang = "E", so I set "J" instead.
...
The second rank Utsunomiya-city consumed 7.4 percent less than top Hamamatsu-city. Is this difference statistically significant? Imagine two bell shape curves, left and right. There are overlap area in the left curve's right slope and the right curve's left slope. 1 standard error from each (total 6 percent) does not overlap, but 2 standard error (total 12 percent) overlap. 

```{r confirm, echo=FALSE}
# 010920070 371 ぎょうざ
top10city <- expenditure %>% 
  filter(cat01_code == "010920070") %>% 
  filter(year == 2018, area_code != "00000") %>% 
  select(city, value) %>% 
  arrange(desc(value)) %>% 
  top_n(10)
...
The battle between Hamamatsu-city and Utsunomiya-city is not just in 2018, but has a long history, according to the table in Hamamatsu-city's victory declaration [(Japanese)](https://prtimes.jp/main/html/rd/p/000000009.000038874.html). Let us confirm it by drawing all available years of top 10 cities in 2018.
...
Hamamatsu-city's victory declaration [(Japanese)](https://prtimes.jp/main/html/rd/p/000000009.000038874.html) also says that Hamamatsu-city has been the top of eating the grilled eel in 11 years in a row. Let us confirm it.
...
I show the top 3 in eel every year, and confirm Hamamatsu-city's claim again.
...
```
As Hamamatsu-city joined non-prefectural-capital large cities in April 2007, their numbers began to show up from 2008.

## Persistent victory items are mostly speciality goods

Are there similar items whose consumption are dominated persistently by one city? I count the number of cities which get the top rank in 11 years from 2007 to 2018 for each item.
...
area_ranks %>% 
  filter(ranks == 1, year == 2018) %>% 
  semi_join(n_top_1, by = "cat01_code") %>% 
  select(cat01_code, `品目分類（27年改定）`, city) %>% 
  print(n=Inf)
...
In how many items among `r lookup_cat01 %>% filter(level == 5) %>% nrow()` did each city get the top in 2018?
...
I draw the histogram that shows each city's ranks of `r lookup_cat01 %>% filter(level == 5) %>% nrow()` items in 2018. Naha-city and other Kyushu cities and Wakayama-city tend to rank low, while Tokyo and other Kanto cities tend to rank high.
...
area_ranks %>% 
  ...
  labs(title = "Ranks distribution in 52 cities in 2018")
...
expenditure %>% 
  filter(level == 1) %>% 
  select(cat01_code, `品目分類（27年改定）`) %>% 
  unique()

national <- expenditure %>% 
  filter(cat01_code == "001100000") %>%
  filter(year == 2018, area_code == "00000", cat02_code == "03") %>% 
  select(value) %>%
  as.numeric()
...
expenditure %>% 
  ...
  geom_hline(yintercept = national,
             color = "white", size = 2) +
  geom_point() +
  annotate("text", x = "34100 広島市", y = national + 190000,
           label = "national mean") +
  coord_flip() +
  labs(
...
cities %>% 
  write.csv("data/cities.csv", row.names = FALSE, fileEncoding = "CP932")
